...
class SessionStats:
    ...
    def __init__(self, session_id: str, session: StudySession):
        ...
        # Set True by posture_monitor once the calibration attempt has
        # concluded (successfully, unsuccessfully, or skipped because
        # the webcam never opened) -- see status_writer, which must not
        # report phase=ACTIVE while a session is still CALIBRATING.
...
def _open_webcam(preferred_index: int):
    """
    Opens the webcam robustly instead of trusting a single hardcoded
    index. Tries `preferred_index` (CAMERA_INDEX from config.py, index
    0 by default) first; if that particular index is misconfigured
    (e.g. set to a non-zero index that doesn't exist on this machine)
    falls back to plain index 0, since that's the overwhelmingly
    common "default/only webcam" index. Returns an opened
    cv2.VideoCapture, or None if no camera could be opened at all --
    never raises.
    """
    try:
        cap = cv2.VideoCapture(preferred_index)
    except Exception as e:
        print(f"[CAMERA] ERROR: exception opening camera index {preferred_index}: {e!r}")
        cap = None
    ...
    if preferred_index != 0:
        print(f"[CAMERA] Camera index {preferred_index} unavailable -- retrying with index 0")
        if cap is not None:
            cap.release()
        try:
            cap = cv2.VideoCapture(0)
        except Exception as e:
            print(f"[CAMERA] ERROR: exception opening camera index 0: {e!r}")
            return None
        if cap is not None and cap.isOpened():
            return cap
...
def posture_monitor(stats: SessionStats):
    ...
    print(f"[CAMERA] VideoCapture opened: {cam_opened}")
    if not cam_opened:
        print("[CAMERA] ERROR: Unable to open webcam")
        print("Webcam unavailable.\nPosture monitoring disabled.")
        log_event(stats.session_id, "WEBCAM_UNAVAILABLE")
        session_bridge.publish_state(calibration_progress=1.0, error="webcam_unavailable")
    ...
    while not calibrated and calibration_attempts < 100 and not _stop_event.is_set():
        ret, frame = cap.read()
        if ret:
            # MediaPipe initialization happens lazily on the first call
            # into tracker.calibrate() (see posture_tracker._get_pose).
            # Guarded here so a MediaPipe load/inference error is
            # logged clearly and skipped for this frame instead of
            # silently killing the whole camera thread -- the webcam
            # preview (write_frame below) must keep working even if
            # pose detection itself can't.
            try:
                calibrated = tracker.calibrate(frame)
            except Exception as e:
                print(f"[MEDIAPIPE] ERROR: {e!r}")
    ...
    stats.calibration_done = True
    print("[CAMERA] Camera processing loop started")
    print("[SESSION] Camera processing started")
    ...
    try:
        while not _stop_event.is_set():
            ...
            # Same guard as during calibration above: a MediaPipe
            # error on one frame must not take down the whole camera
            # loop (and with it, the live preview / webcam release).
            try:
                posture_state = tracker.update(frame)
            except Exception as e:
                print(f"[MEDIAPIPE] ERROR: {e!r}")
                posture_state = stats.posture_state or UNKNOWN
...
def status_writer(stats: SessionStats):
    ...
    while not _stop_event.is_set():
        ...
        if stats.duration_seconds is not None:
            ...
            # Don't publish ACTIVE/PAUSED until posture_monitor has
            # actually finished its calibration attempt (success,
            # failure, or "no webcam" all count as "concluded") --
            # otherwise this loop (which starts ticking immediately,
            # in parallel with the webcam/MediaPipe init that can take
            # a few seconds) races ahead and flips the Live Session
            # page out of the CALIBRATING screen before calibration
            # has actually run, before the "webcam did or didn't open"
            # outcome even happened.
            if stats.calibration_done:
                fields["phase"] = (session_bridge.PHASE_PAUSED if _pause_event.is_set()
                                    else session_bridge.PHASE_ACTIVE)
            session_bridge.publish_state(**fields)
